<x-kitchen-layout title="Rekap Bahan Per Tanggal">
    <div class="mb-6 hidden print:block">
        ...
        <p class="mt-1 text-sm text-nk-muted">Tanggal produksi: {{ \Carbon\Carbon::parse($selectedDate)->format('d M Y') }}</p>
    </div>
    ...
    <div class="print:hidden">
        ...
        <x-card class="mb-5" padding="sm">
            <form method="GET" class="grid gap-3 md:grid-cols-3">
                <x-input type="date" name="date" label="Tanggal produksi" :value="$selectedDate" />
    ...
    <div class="grid gap-4 md:grid-cols-3">
        <x-card padding="lg"><p class="text-sm text-nk-muted">Tanggal</p><p class="mt-2 font-heading text-3xl text-nk-primary">{{ \Carbon\Carbon::parse($selectedDate)->format('d M Y') }}</p></x-card>
    ...
    <section class="mt-6">
        ...
        <x-admin-table>
            <thead class="bg-nk-alt/70 text-xs uppercase tracking-wide text-nk-muted"><tr><th class="px-4 py-3">Invoice</th><th class="px-4 py-3">Customer</th><th class="px-4 py-3">Jam acara</th><th class="px-4 py-3">Menu ringkas</th><th class="px-4 py-3">Status</th></tr></thead>
            <tbody>
            ...
            @empty
                <tr><td colspan="5" class="px-4 py-6 text-center text-nk-muted">Tidak ada order produksi pada tanggal ini.</td></tr>
            @endforelse
            </tbody>
        </x-admin-table>
    </section>
